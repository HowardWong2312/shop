$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'goods/shopGoodsOnebuy/list',
        datatype: "json",
        colModel: [			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(300).queue(function() {
                let url = vm.shopGoodsOnebuy.id == null ? "goods/shopGoodsOnebuy/save" : "goods/shopGoodsOnebuy/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.shopGoodsOnebuy),
                    success: function(r){
                        if(r.code == 200){
        ...
        success: function () {
            vm.shopGoodsOnebuy.status = 1;
            vm.subCheck("通过")
        },
        refuse: function () {
            vm.shopGoodsOnebuy.status = 2;
            vm.subCheck("拒绝")
        },
        subCheck: function (title) {
            ...
            layer.confirm('确定要对该活动申请执行 '+title+' 操作？', {
                ...
                if(!lock) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: baseURL + "goods/shopGoodsOnebuy/check",
                        contentType: "application/json",
                        data: JSON.stringify(vm.shopGoodsOnebuy),
                        success: function(r){
                            if(r.code == 200){
		...
		del: function () {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "goods/shopGoodsOnebuy/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "goods/shopGoodsOnebuy/info/"+id, function(r){
                vm.shopGoodsOnebuy = r.shopGoodsOnebuy;
            });
		},
        getStatusList: function () {
            $.get(baseURL + "sys/dict/list/goods_onebuy_status", function (r) {
